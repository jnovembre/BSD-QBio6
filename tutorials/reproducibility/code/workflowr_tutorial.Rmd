...
use to share your work. Please follow the [setup instructions](#setup) in the next section prior to the workshop.
...
Note on paths: When you start or open a workflowr project in RStudio
(e.g. by clicking on `myproject.Rproj`) RStudio will set the "working directory" 
to the location of the workflowr project on your computer. (You can run `getwd()` in the R console to check this if you like.)
This means that to save (or load) a file in the data subdirectory you need to type `data/` before the name of the file. Note that you don't need
to give the "full path". That is you don't need to type something like
    ```{r}
    write.csv(ToothGrowth, file = "C:/Users/GraceHopper/myproject/data/teeth.csv")
    ```

Specifying a file name like "data/teeth.csv", is called using
a "relative path" because
it specifies the file name relative to the current working directory.
You should use relative paths where possible because it can help
make your code easier for others to run and to run on different computers
...
1. In the R console, open a new R Markdown file by typing 
    ...
    ```
It should create and open a new `.Rmd` file for editing in RStudio. The file looks pretty much like other `.Rmd` files, but in the header 
note that workflowr provides its own custom output format, `workflowr::wflow_html`. The only other minor difference is that `wflow_open()` adds the editor option `chunk_output_type: console`, which causes the code to be
...
`teeth.Rmd`, and execute it. Try executing it several tiems -- does it give you a different answer each time?
