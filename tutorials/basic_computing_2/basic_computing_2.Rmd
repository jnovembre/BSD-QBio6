...
patterns repeated in Illinois? Using `subset`, and copying some of the
code above, we can investigate this question:

```{r tornadoes-2}
temp   <- subset(tornadoes,STATE == "ILLINOIS")
...
As we continue to develop our analysis, can also functions reduce the
effort of making revisions; for example, if we want to change the
colour of the lines in all the plots, this requires only one small
change to the code inside `plot_events`.
...
[`https://stefanoallesina.github.io/BSD-QBio4`](https://stefanoallesina.github.io/BSD-QBio4)
and follow the link "Submit solution to challenge 2" to submit your
answer (alternatively, you can go directly to
[`goo.gl/forms/QJhKmdGqRCIuGNPa2`](https://goo.gl/forms/QJhKmdGqRCIuGNPa2)).
