...
******
#### Exercises 1.1 and 1.2 --- Load libraries
For exercises 1.1 and 1.2, we will need ggplot2 and Tsne. Let's load the R packages:

```{r, tidy = 'formatR',include=TRUE}
library("ggplot2")
library("Rtsne")
```

As a general rule of thumb, if you get an error message saying there is no package titled ggplot2 or Rtsne you may need to first install the appropriate package: 
...
In order to visualize our scRNA-seq data using PCA, we will need to use some functions from ggplot2.
...
******
#### Exercises 2.1 and 2.2 --- Load libraries
For exercises 2.1 and 2.2, we will need ggplot2 and ggseqlogo Let's load the R packages:
...
# load data
iedb <- read.csv('./data/bdata.20130222.mhci.txt', header = TRUE, sep = "\t",as.is = TRUE)
...
listOfSequences = filtered_iedb[,4]
 ...
 # number of sequences
  ...
  # create empty PFM matrix of zeros with dimensions of 20 x length of each sequence
  PFM = matrix(0L, length(aminoAcidsVocab), lengthOfSequence)
  
  # for loop through each sequence
  for (sequence in listOfSequences) {
    
    # transform character vector to vector of single characters for looping
    sequenceString = strsplit(sequence, "")[[1]]
    
    # for loop through each amino acid in sequence
    for (index in seq_along(sequenceString)) {
      
      # increment value at PFM[amino acid at position, positon]
      PFM[match(sequenceString[index], aminoAcidsVocab), index] = PFM[match(sequenceString[index], aminoAcidsVocab), index] + 1
    }
  }
...
```{r, tidy = 'formatR',include=TRUE}
ggseqlogo(PFM, seq_type='aa', as.is=TRUE)
```
******
